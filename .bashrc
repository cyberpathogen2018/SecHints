...
#alias myip="echo;ip -c a | grep -A 5 'tun0' | grep -w 'inet' | cut -d '/' -f1;echo"
...
function dockershellhere() {
	dirname=${PWD##*/}
        docker run --rm -it --cap-add=SYS_PTRACE --security-opt seccomp=unconfined --entrypoint=/bin/bash -v `pwd`:/${dirname} -w /${dirname} "$@"
}
